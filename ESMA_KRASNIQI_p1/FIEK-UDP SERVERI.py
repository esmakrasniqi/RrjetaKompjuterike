...
while vlera:
    try:
        Operacioni, clientAddress = serverSocket.recvfrom(2048)
        kushti = Operacioni.decode("ASCII")
        socketName = socket.getsockname(serverSocket)
        print(str(clientAddress[0]) + ":" + str(clientAddress[1]) + " Operacioni " + kushti.split(" ")[0] + " eshte pranuar...")
    except:
        continue
        pass

    if kushti == "IPADRESA":
        pergjigjja =  "IP adresa e klientit eshte " + str(clientAddress[0])
    ...
    try:
        serverSocket.sendto(pergjigjja.encode("ASCII"), clientAddress)
        print( clientAddress[0] + ":" + str(clientAddress[1]) + " pergjigjja " + kushti.split(" ")[0] + " eshte derguar...")
    except:
        print(clientAddress[0] + ":" + str(clientAddress[1]) + " pergjigjja nuk mund te dergohet!")
        pass
